...
- **rules/analyzers** apply the heuristics shipped in `milstdlint/core.py`.
- **score** ranks by severity.
- **MCP server** (`milstdlint mcp`) exposes `scan` for Cognis.Studio agents.
